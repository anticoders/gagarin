...
module.exports.execute = function (code, args) {
  ...
  return self.__custom__(function (operand, done) {
    ...
    // NOTE: it's very important for this action to be idempotent,
    //       because it may be repeated when it fails for the first time

    var context = this;
    var closure = operand.closure ? operand.closure() : {};

    operand.browser.execute("return (" + wrapSourceCode(codeToString(code), clean(context, args).map(stringify), context, closure) + ").apply(null, arguments)",
...
module.exports.promise = function (code, args) {
  ...
  return self.__custom__(function (operand, done) {
    ...
    var myArgs = clean(context, args).map(stringify); // stringify arguments

    myArgs.unshift("function ($) { setTimeout(function () { cb({ context: context, closure: closure(), error: ($ && typeof $ === 'object') ? $.message : $.toString() }) }) }");
    myArgs.unshift("function ($) { setTimeout(function () { cb({ context: context, closure: closure(), value: $ }) }) }");
    ...
    chunks.push(
      ...
      "      action.apply(context, [" + myArgs.join(", ") + "]);",
      "    } catch (err) {",
...
module.exports.wait = function (timeout, message, code, args) {
  ...
  return self.__custom__(function (operand, done) {
    ...
    var myArgs = clean(context, args).map(stringify);
    ...
    chunks.push(
      ...
      '      var value;',
      '      try {',
      '        value = action.apply(context, [' + myArgs.join(', ') + ']);',
      '        if (value) {',
