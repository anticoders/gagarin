...
module.exports = {
  ...
  /**
  * proof of concept loadScript helper
  * should only load from file once per test suite
  * @param {String} path -  URL , e.g. 'http://www.google.com'
  * @param {String} verify - property name which must exist to verify this script has loaded and initalized
  * @param {Object} initFunction - invoked after the script has been verified 
  */
  loadScript: function(path, verify, initFunction ) {

    var code;
    if(loadedScripts[path] && loadedScripts[path]['code']){
      code = loadedScripts[path]['code'];
    }else{
      code = fs.readFileSync(path, "utf8");
      loadedScripts[path]={};
      loadedScripts[path]['path']=path;
      loadedScripts[path]['code']=code;
      //TODO handle when these are not provided, they should be optional
      loadedScripts[path]['verify']=verify;
      loadedScripts[path]['initFunction']=initFunction;
    }

    return this.promise(function (resolve, reject, code, verify, initFunction) {
      var script = document.createElement('script');
      script.text = code;
      window.document.head.appendChild(script);
      //TODO eval or execute the verify statement
      if(!!verify){
        initFunction();
        resolve();
      }else{
    ...
    }, [ code, verify, initFunction ]);
  },

  /**
  * proof of concept reloadScripts() 
  * TODO should only re-load the scripts for the current test suite
  * this load all previously loaded scripts
  * it could potentially be invoked internally after getURL and page refresh
  * or the user could invoke it directly in their tests, depends on
  */

  reloadScripts : function(timeout){
    
    var scripts = [];
    var self = this;
    for (var key in loadedScripts) {
      var path = loadedScripts[key]['path'];
      var verify = loadedScripts[key]['verify'];
      var initFunction = loadedScripts[key]['initFunction'];
      scripts.push( this.loadScript(path,verify,initFunction) );
    }
    
    // I can't figure out how to resolve all of these loadScript promise instances
    Promise.all(scripts).then(function(arr){
    });

    // before we resolve this
    return self.promise(function(resolve){
      resolve();
    });
  ...
  /**
  * proof of concept getURL
  * this would be the case where reload is called automatically
  * the alternative is to make the user call reloadScripts manually
  */

  getURL : function(url,timeout){
    var self = this;
    return this.wait(timeout || DEFAULT_TIMEOUT, 'until ' + url + ' is ready', function (url) {
      window.location.href = url;
      return !!window;
    }, [ url ])
    .then(function(value) {
      return self.reloadScripts();
    }, function(reason) {
