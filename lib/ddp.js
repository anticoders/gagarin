...
module.exports = function makeDDPClientFactory (emitter, requestGagarinConfig) {
  "use strict";
        ...
        ddpClient.connect(function (err, wasReconnected) {
          ...
          resolve({
            ...
            execute: function (code, args, cb) {
              callDDPMethod('/gagarin/execute', [ code.toString(), args, config.closure() ], cb);
            },

            promise: function (code, args, cb) {
              callDDPMethod('/gagarin/promise', [ code.toString(), args, config.closure() ], cb);
            },

            wait: function (timeout, message, code, args, cb) {
              callDDPMethod('/gagarin/wait', [ timeout, message, code.toString(), args, config.closure() ], cb);
            },
