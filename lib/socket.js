    ...
    return requestGagarinConfig().then(function (config) {
      ...
      socketPromise = new Promise(function (resolve) {
        ...
        function transmit (payload, callback) {
          payload.closure = config.closure();
          payload.closureKeys = Object.keys(payload.closure);
          // TODO: use EJSON
        ...
        socket.on('data', function (chunk) {
          ...
          chunk.split('\n').forEach(function (line) {
            ...
            if (data.error) {
              ...
              if (data.ping) {
                transmit({ name : data.name, mode : 'pong' });

              } else {
                // XXX note that the first argument must be an error
                emitter.emit(data.name, null, data.result);
              }
