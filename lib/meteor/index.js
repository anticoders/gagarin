...
module.exports = function createMeteor (options) {
  ...
  myPrototype.init = function (config) {
    getConfig = _(function () {
      return checkIfVersionsMatch(options.pathToApp).then(function () {
        return config;
      });
    }).memoize(); // the version check will only be executed once
    return this;
  }
  ...
  function getMeteor () {
    ...
    return Promise.all([
      ...
      getPathToMain(), getMongoUrl(), tools.getNodePath(pathToApp)

    ]).then(function (results) {

      return getMeteorProcess({
        pathToNode : results[2],
