...
module.exports = function createMeteor (options) {
  ...
  meteor.useFixture = function (name, where) {
    // TODO: verify arguments when object is given
    var fixture = typeof name === 'object' ? name : fixtures.getByName(name);
    if (typeof name !== 'string' && typeof name !== 'object') {
      throw new Error('first arguments to useFixture must be either string or object');
    }
    if (!fixture) {
      throw new Error('fixture ' + name + ' does has not been registered yet');
    }
    if (!where) {
      where = [ 'client', 'server' ];
    }
    ...
    where.forEach(function (target) {
      if (target === 'client') {
        if (clientFixtures[name]) {
          // this fixture is already used
          return;
        }
        clientFixtures[name] = true;
      } else if (target === 'server') {
        if (serverFixtures[name]) {
          // this fixture is already used
          return;
        }
        serverFixtures[name] = true;
      } else {
      ...
      listOfFixtures.push({
        ...
        line  : fixture.lineNumber,
        code  : fixture.sourceCode,
      });
    })
  }
  ...
  meteor.useFixtures = function (pathToFixtures, regexp, where) {
    ...
    (function lookForFixtures(pathToFile) {
      ...
      var split = null;
      var whereToPutThatFile = where;
      var relativePath = path.relative(pathToFixtures, pathToFile);
      ...
      if (!fs.lstatSync(pathToFile).isDirectory()) {
        ...
        split = pathToFile.split(path.sep);
        if (split[0] === 'client' || split[0] === 'server') {
          // overwrite the default "where"
          whereToPutThatFile = [ split[0] ];
        }
        whereToPutThatFile.forEach(function (target) {
  ...
  meteor.useFileAsFixture = function (name, filePath, where) {
    if (fixtures.getByName(name)) {
      console.warn('useFileAsFixture: the fixture ' + name + ' is already registered')
    }
    var sourceCode = fs.readFileSync(path.resolve(filePath), 'utf8');
    meteor.useFixture({
      fileName      : name,
      lineNumber    : 1,
  ...
  meteor.addJavaScript = function (where, options, code) {
    // TODO: verify arguments

    if (!codeFixturesAllowed) {
      throw new Error('you can only inject source code before server starts');
    }
    ...
    var myPath = 'fixtures.js';

    if (options.toPackage) {
      myPath = path.join('packages', options.toPackage + '.js');
    }

    if (options.toFile) {
      myPath = path.join('app', options.toFile + '.js');
    }
    ...
    listOfFixtures.push({
      ...
      path  : myPath,
      code  : code,
      file  : options.fromFile     !== undefined ? options.fromFile     : stack()[1].getFileName(),
      line  : options.atLineNumber !== undefined ? options.atLineNumber : stack()[1].getLineNumber(),
    });
  }
