    ...
    meteorPromise = new Promise(function (resolve, reject) {
      ...
      cleanUpThen(function () {
        ...
        (meteorPort ? Promise.resolve(meteorPort) : findAvailablePort()).then(function (port) {
          ...
          var dest = path.join(options.pathToApp || module.exports.getUserHome(), '.gagarin','temp'+id);
          if (codeFixtures && codeFixtures.length > 0) {
            isolateCode(pathToBuild, dest,function(){
              applyCodeFixtures(codeFixtures, dest, function (err) {
                if (err) {
                  reject(err);
                } else {
                  spawnProcess();
          ...
          function spawnProcess () {
            setTimeout(function () {
              meteorProcess = new createMeteorProcess(pathToNode, dest+'/main.js', env, meteorProcessPrototype, options,id);
            }, meteorRestartDelay);
          }
