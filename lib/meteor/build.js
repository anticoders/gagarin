...
function BuildPromise(options) {
  ...
  return findAvailablePort().then(function (port) { return new Promise(function (resolve, reject) {
    ...
    var meteor = spawnMe('meteor', [
      'build', '--debug', '--directory', pathToLocal,
    ...
    meteor.on('exit', function (code) {
      tools.getNpmPath(pathToApp).then(function (pathToNpmBinary) {
        var npm = spawnMe(pathToNpmBinary, [ 'install' ], {
          cwd: path.join(pathToLocal, 'bundle', 'programs', 'server')
        });
