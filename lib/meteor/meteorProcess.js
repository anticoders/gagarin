...
module.exports = function createMeteorProcess (node, main, env, myPrototype, options) {
  ...
  var safetyTimeout  = options.safetyTimeout;
      ...
      kill(function (err) {
        if (instance.onStart) {
          if (err) {
            ...
            instance.onStart(new Error(safetyTimeout +'ms safety timeout exceeded when waiting for any server output' +
            ' please try to increase it using -T option'));
          }
        }
      });
    }, safetyTimeout);
    //just for internal test
    if(options.serverStartupTimeout)
      safetyTimeout = options.serverStartupTimeout;
          ...
          kill(function (err) {
            if (instance.onStart) {
              if (err) {
                ...
                instance.onStart(new Error(safetyTimeout +'ms safety timeout exceeded when waiting for server startup' +
                ' please try to increase it using -T option'));
              }
            }
          });
