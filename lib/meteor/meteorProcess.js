...
module.exports = function createMeteorProcess (node, main, env, myPrototype, options, id) {
  ...
  setTimeout(function () {
    ...
    // TODO: cancel these tasks
    cleanup.addTask(asyncRemoveTemporaryCopy);
    cleanup.addTask(onProcessExit);
    ...
    // XXX: it's enough to add this to cleanup jobs ...
  ...
  */
  ...
  function onProcessExit(done) {
    // only kill if the process is still running
    if (meteor) {
      this.describe('killing a meteor process listening at ' + env.PORT);
      meteor.kill();
      meteor = null;
    }
    done();
  ...
  function asyncRemoveTemporaryCopy(done){
    // TODO: it's possible that this path does not really exists
    // TODO: is it really the right place to delete this?
    var pathToTemp = path.join(options.pathToApp || module.exports.getUserHome(), '.gagarin', 'local', 'build_' + id);
    rimraf(pathToTemp, done);
  }
