...
var createTestResultsServer = require('../common/testResultsServer');
...
function Gagarin (options) {
  ...
  function getMocha() {
    ...
    mocha._reporter = function (runner, options) {
      ...
      createTestResultsServer.registerTestRunner(mocha.suite, runner);

      // install the velocity reporter listeners if needed ...
      velocity && velocity(runner);
    ...
    function update() {
      ...
      try {
        next.run(function (numberOfFailures) {
          createTestResultsServer.unregisterTestRunner(next.suite);
          //-------------------------------------------------------
