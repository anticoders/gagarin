  ...
  suite.on('pre-require', function (context) {
    ...
    context.meteor = function (options, onStart) {
      ...
      if (!options.noAutoStart) {
        ...
        before(function () {
          return meteor.init().startup(onStart);
        });
      ...
      if (options.mocha) {
        ...
        meteor.addJavaScript('client', { fromFile: 'browser-source-map-support.js', atLineNumber: 1 },
          fs.readFileSync(path.resolve(__dirname, '../../node_modules/source-map-support/browser-source-map-support.js'), 'utf8'));

        meteor.addJavaScript('client', { fromFile: 'mocha.js', atLineNumber: 1 },
          fs.readFileSync(path.resolve(__dirname, '../../node_modules/mocha/mocha.js'), 'utf8'));
        
        meteor.addJavaScript('server', { fromFile: 'gagarin/mocha.js', atLineNumber: 1 }, fixtures.get('mocha'));
        meteor.addJavaScript('client', { fromFile: 'gagarin/mocha.js', atLineNumber: 1 }, fixtures.get('mocha'));
      }

      if (options.flavor == "fiber") {
        var proxy = wrapPromisesForFiber(meteor, meteor.methods);
        proxy.getDDPSetup   = meteor.getDDPSetup;
        proxy.addJavaScript = meteor.addJavaScript;
        return proxy;
      } else {
        return meteor;
      }
