...
var mocha = new _Mocha({ reporter: Laika });
...
function Laika(runner) {
  ...
  runner.on('start', function() {
    Gagarin.feedback && Gagarin.feedback('start', { total: total });
  });

  runner.on('pass', function(test){
    Gagarin.feedback && Gagarin.feedback('pass', clean(test));
  });

  runner.on('fail', function(test, err){
    Gagarin.feedback && Gagarin.feedback('fail', clean(test));
  });
  ...
  runner.on('end', function(){
    ...
    Gagarin.feedback && Gagarin.feedback('end', self.stats);
  });
}
