...
function Meteor (options) {
  ...
  this.meteorProcessAsPromise = Promise.all([
    ...
    return new Promise(function (resolve, reject) {
      ...
      function requestGagarinConfig (meteorNeedRestart, meteorRestartTimeout) { // and callback

        var callback = arguments[arguments.length-1];

        if (arguments.length < 3) {
          meteorRestartTimeout = undefined;
        }

        if (arguments.length < 2) {
          meteorNeedRestart = undefined;
        }
...
MeteorPromiseChain.methods.forEach(function (name) {
  ...
  Meteor.prototype[name] = function () {
    var chain = new MeteorPromiseChain(this.meteorProcessAsPromise, this.helpers);
