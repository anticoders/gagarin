...
module.exports = {
  ...
  getPathToDevBundle: _.memoize(function (releaseName) {
    "use strict";

    var version = parseRelease(releaseName);
    var home = module.exports.getUserHome();

    if (version < "0.9.0") {
      return module.exports.getReleaseConfig(version).then(function (config) {
        return path.join(home, '.meteor', 'tools', config.tools);
      });
    }

    var getMeteorToolPath = require('./meteorToolPath');

    return new Promise(function (resolve, reject) {
      getMeteorToolPath(home, releaseName, function (err, toolPath) {
        if (err) {
          reject(err);
        } else {
          resolve(path.join(toolPath, 'dev_bundle'));
        }
      });
    });
  ...
  getMongoPath: _.memoize(function (pathToApp) {
    "use strict";
    return module.exports.getReleaseName(pathToApp)
      .then(function (releaseName) {
        return module.exports.getPathToDevBundle(releaseName)
  ...
  smartPackagesAsPromise: function (pathToApp) {
    ...
    console.log([
      chalk.red('========================================================================================'),      
      chalk.red('we have detected a smart.json file in ' + pathToApp),
      chalk.red('since Gagarin no longer supports meteorite, this file will be ignored'),
      chalk.red('========================================================================================'),
    ].join('\n'));
