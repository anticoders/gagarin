...
/**
 * Apply (inject) code fixtures into the given build.
 *
 * @param {object[]} codeFixtures

 * @param {string}   pathToBuild
 * @param {function} done
 */
export default function getModifiedBundle (codeFixtures, pathToBuild, done) {
...
function writeProgramJSON (pathToBuild, where, programJSON) {
  ...
  var res = fs.writeFileSync(pathToProgramJSON, JSON.stringify(programJSON, undefined, 2));
}
